...
///le paso un post Irom , le pide un psit repostirory , le pido q cree un queri
      let postTypeORM: PostTypeORM = await this.postRepository
        .createQueryBuilder()
        //el 1er es para id de la tabla post ,le paso un post id, que escuche el notificatioon, cuando sea aceptet emite un evento y el type scrpt escuha
        .where("id = :id")
